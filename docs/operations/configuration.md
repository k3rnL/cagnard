...
Cagnard is configured by one HOCON document. The backend does not require a database: users, authentication policy, provider accounts, roots, access rules, appearance defaults, and UI opener declarations are reconstructed from configuration at startup.
