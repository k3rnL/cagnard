...
First-party file openers use scoped Cagnard file APIs and never receive provider credentials. Structured-data parsing and queries run in one local worker runtime per tab with isolated source IDs, connections, generations, cancellation, and cleanup. Results remain bounded. SQL is parsed and allowlisted to one read-only statement over `data`; mutation, commands, table functions, extra relations, URLs, and unknown AST nodes are rejected. DuckDB automatic extension installation and loading are disabled. Materialized query sources disable external access; Parquet and Iceberg narrow access to their authorized same-origin object or table facade.

Iceberg metadata is security-sensitive because it contains object references. The backend validates table location, manifests, metadata logs, and statistics paths and serves approved objects through a root-confined, authorization-checked, same-origin facade. Foreign buckets, credentials, external schemes, absolute filesystem paths, traversal, and encoded escapes are denied. The browser never receives storage account secrets.

NetCDF-C Wasm currently requires a full buffer, so the backend-published source size and configured `maxSourceBytes` are enforced before parsing. Hyperslab, projection, plot, export, and worker-message bounds are enforced separately. Generic HDF5 is not accepted as NetCDF without semantic validation.

The packaged frontend Content Security Policy permits local scripts, Wasm, workers, media, and same-origin API access; it does not permit arbitrary remote data connections. The main application does not allow JavaScript evaluation. The dedicated structured-data worker receives a narrower response policy that permits `unsafe-eval` because the pinned Apache Arrow validity builder uses generated predicates; that exception is confined to the isolated worker and must be revalidated when Arrow changes. The shared runtime is terminated on logout and page teardown. Cagnard does not load arbitrary third-party executable frontend bundles from configuration. Treat any future executable UI or provider plugin as trusted code requiring review and isolation.
